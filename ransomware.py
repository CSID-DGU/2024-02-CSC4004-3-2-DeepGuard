...
# 실행
directory_path = r"C:\Users\sec\Downloads"  # .exe 파일이 저장된 디렉토리 경로
    ...
    def on_created(self, event):
        if event.src_path.endswith(".exe"):
            ...
            if features:
                ...
                prediction_results = predict_ransomware(features_df, self.model, self.feature_columns, self.threshold)
                print(prediction_results)
                # 추가 작업: 로그 저장, 결과 알림 등
                prediction_results.to_csv("real_time_prediction_log.csv", mode='a', header=False, index=False)
